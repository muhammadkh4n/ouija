...
function stampInstanceId<T>(
  ...
  if (
    ...
    return { ...(payload as unknown as object), instanceId: instance.id } as unknown as T;
  }
  if (topic === 'pipeline.manually_dispatched') {
    // Manual dispatch also wants `cardId` stamped — the synthetic
    // `manual/<uuid>` is generated by the orchestrator at instance creation
    // time, so the pure transition cannot include it.
    return {
...
export class Orchestrator {
  ...
  private async applyTrigger(
    ...
    const eventRecords = outcome.events.map((e, i) => ({
      ...
      payload: stampInstanceId(e.topic, e.payload, instance),
      occurredAt: now,
      sequence: seqBase + i,
    }));
  ...
  private async _executeSideEffect(
    ...
    switch (effect.type) {
      case 'dispatch_agent': {
        ...
        // Manual-dispatch paths (`requestManualDispatch`) attach the
        // human-meaningful task title inline so the worker doesn't have to
        // call kanbanPlugin.getCard for synthetic `manual/<uuid>` cardIds.
        // The `taskTitle` field on AgentDispatchJobData was reserved exactly
        // for this — see `packages/bus/src/job-queue.ts`.
        const taskTitle = effect.payload['taskTitle'];
        if (typeof taskTitle === 'string' && taskTitle.length > 0) {
          dispatchJobData.taskTitle = taskTitle;
        }
  ...
  /**
   * Administrative dispatch — bypass the kanban round-trip. Pre-creates a
   * fresh `idle` pipeline instance with a synthetic `manual/<uuid>` cardId,
   * then applies the {@link PipelineTrigger} `manual_dispatch` to drive it
   * into `dispatching`. Used by `POST /api/v1/pipelines/dispatch` and
   * (Phase 3+) by `ouija watch`.
   *
   * `boardId` resolution: the route forwards an optional override; when
   * omitted, this method picks the sole configured board and errors out if
   * the deployment has more than one (so single-board self-hosters get a
   * one-click flow without surfacing a board picker for everyone).
   *
   * 6th `applyTrigger` caller — closes the "4 entry points share applyTrigger"
   * acceptance criterion the phase note carried since Task 1 (now 6 entry
   * points: kanban, review-loop, stall, admin_reset, dwell-reconciler,
   * manual-dispatch — all routed through one persist-+-side-effect path).
   */
  async requestManualDispatch(input: {
    agentId: string;
    title: string;
    description: string;
    boardId?: string;
    ...
    const trimmedAgent = input.agentId.trim();
    if (trimmedAgent.length === 0) {
      return { kind: 'rejected', reason: 'agentId is required' };
    }
    if (trimmedTitle.length === 0) {
      return { kind: 'rejected', reason: 'title is required' };
    }
    if (trimmedDesc.length === 0) {
      return { kind: 'rejected', reason: 'description is required' };
    }

    let boardId: import('@ouija-dev/types').BoardId;
    if (input.boardId !== undefined && input.boardId.length > 0) {
      boardId = input.boardId as unknown as import('@ouija-dev/types').BoardId;
    } else {
      const all = await this.db.boardConfigs.listAll();
      if (all.length === 0) return { kind: 'no_board' };
      if (all.length > 1) {
        return {
          kind: 'ambiguous_board',
          boardIds: all.map((c) => String(c.boardId)),
        };
      }
      boardId = all[0]!.boardId;
    }

    const config = await this._getConfig(boardId);
    if (config === undefined) {
      return { kind: 'config_missing', boardId: String(boardId) };
    }

    // Synthesize the instance directly — `_createIdleInstance` goes through
    // `kanbanPlugin.getCard` which would fail on a manual cardId.
    const now = new Date().toISOString();
    const instanceId = makeInstanceId(randomUUID());
    const cardId = `manual/${randomUUID()}` as unknown as import('@ouija-dev/types').CardId;
    const newInstance: import('@ouija-dev/types').PipelineInstance = {
      id: instanceId,
      cardId,
      boardId,
      // projectId mirrors `_createIdleInstance` until project mapping is wired.
    ...
    const result = await this.applyTrigger(newInstance, trigger, config);

    if (!result.accepted) {
      this.logger.info('requestManualDispatch: transition rejected', {
    ...
    // After applyTrigger, the instance's state has the dispatchId. Re-read
    // for the response so the caller can correlate.
    const post = await this.db.pipelines.findById(instanceId);
    const dispatchId =
      post !== undefined && 'dispatchId' in post.state ? String(post.state.dispatchId) : '';

    this.logger.info('requestManualDispatch: pipeline dispatched', {
      instanceId: String(instanceId),
      cardId: String(cardId),
...
/**
 * Result of {@link Orchestrator.requestManualDispatch}. Encodes every
 * failure mode the route maps to a distinct HTTP status (see
 * `packages/server/src/routes/pipelines.ts`).
 */
export type ManualDispatchOutcome =
  | {
      kind: 'dispatched';
      instanceId: string;
      cardId: string;
  ...
  | { kind: 'no_board' }
  | { kind: 'ambiguous_board'; boardIds: string[] }
  | { kind: 'config_missing'; boardId: string }
  | { kind: 'rejected'; reason: string };
